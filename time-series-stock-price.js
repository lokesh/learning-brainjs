...
rawData.forEach(step => {
    max = Math.max(max, step.high);
    min = Math.min(min, step.low);
})

function remap(value, min, max, newMin = 0, newMax = 1) {
    return ((value - min) / (max - min)) * (newMax - newMin) + newMin;
}

function scaleDown(step) {
    return {
        open: remap(step.open, min, max),
        high: remap(step.high, min, max),
        low: remap(step.low, min, max),
        close: remap(step.close, min, max),
    }
}

function scaleUp(step) {
     return {
        open: remap(step.open, 0, 1, min, max),
        high: remap(step.high, 0, 1, min, max),
        low: remap(step.low, 0, 1, min, max),
        close: remap(step.close, 0, 1, min, max),
    }
}
